...
export default function ApartmentsList({ apartments, locale }: Props) {
  const price = useMemo(() => (locale === Locale.BG ? 'от' : 'from'), [locale])

  const pickBg = useCallback(
    (i: number) => (i % 2 === 0 ? 'bg-[var(--color-one)]' : 'bg-[var(--color-two)]'),
    [],
  )
